    ...
    'videoAttributes': function (context) {
      ...
      this.initialize = function () {
        ...
        var body =
          ...
          '<div class="form-group">' +
          '  <label class="control-label col-xs-3">' + lang.videoAttributes.width + '</label>' +
          '  <div class="input-group col-xs-2">' +
          '    <input type="text" id="note-video-attributes-custom-width" class="note-video-attributes-custom-width form-control" name ="width" value = "450">' +
          '  </div>' +
          '</div>' +
          '<div class="form-group">' +
          '  <label class="control-label col-xs-3">' + lang.videoAttributes.height + '</label>' +
          '  <div class="input-group col-xs-2">' +
          '    <input type="text" id="note-video-attributes-custom-height" class="note-video-attributes-custom-height form-control" name ="height" value = "280">' +
          '  </div>' +
          '</div>' +
          '<div class="form-group">' +
          '  <label class="control-label col-xs-3">' + lang.videoAttributes.startAt + '</label>' +
          '  <div class="input-group col-xs-2">' +
          '    <input type="text" id="note-video-attributes-startAt" class="note-video-attributes-startAt form-control" name ="startAt" value = "00:00:00">' +
          '  </div>' +
          '</div>' +
          '<div class="form-group">' +
      ...
      this.show = function () {
        ...
        this.showLinkDialog(vidInfo).then(function (vidInfo) {
          ...
          var $vid = vidInfo.vidDom,
            ...
            $videoWidth = self.$dialog.find('.note-video-attributes-custom-width'),
            $videoHeight = self.$dialog.find('.note-video-attributes-custom-height'),
            $videoStartAt = self.$dialog.find('.note-video-attributes-startAt'),
            url = $videoHref.val(),
            width = $videoWidth.val(),
            height = $videoHeight.val(),
            startAt = $videoStartAt.val(),
            $videoHTML = $('<div/>');
            if(startAt.includes(':') == true){
              var splitTime = startAt.split(':');
              var hourToSec = parseInt(splitTime[0])*3600;
              var minToSec = parseInt(splitTime[1])*60;
              var sec = splitTime[2];
              startAt = parseInt(hourToSec) + parseInt(minToSec) + parseInt(sec);
            }
          if ($videoSize.val() == 0) {
            var videoWidth = 'auto',
              videoHeight = 'auto';
          ...
          if (ytMatch && ytMatch[1].length === 11) {
            ...
            if (startAt && startAt != 0) urlVars += '&start=' + startAt;
            self.$dialog.find('.note-video-attributes-startAt').val('00:00:00');
            console.log(startAt);
            console.log(urlVars);
            var youtubeId = ytMatch[1];
            $video = $('<iframe>')
              .attr('frameborder', 0)
              .attr('src', 'https://www.youtube.com/embed/' + youtubeId + '?' + urlVars)
          ...
          if ($videoSize.val() == 0 && parseFloat(width) == "" && parseFloat(height) == "") $video.addClass('embed-responsive'); else $video.css({ 'float': $videoAlignment.val() });
